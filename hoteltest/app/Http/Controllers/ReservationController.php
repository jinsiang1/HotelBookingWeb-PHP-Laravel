...
class ReservationController extends Controller
{
    public function showForm()
    {
        ...
        $response = Http::get('http://127.0.0.1:8081/api/reservation');
            ...
            log::error('Fail to fetch service from API:'. $response->status());
            $serviceData = [];
        }
    ...
    public function submitReservation(Request $request)
    {
        ...
        $validateDateResult = $this->validateDate($request);
        if($validateDateResult!==true){
    ...
    if ($checkInDate < $today) {
        // If check-in date is before today, show an error message
        return redirect()->back()->with('error', 'Check-in date must be today or a future date.');
    }

    if ($checkOutDate < $checkInDate) {
        // If check-out date is before check-in date, show an error message
        return redirect()->back()->with('error', 'Check-out date must be on or after the check-in date.');
    }
